              ...
              {rooms.map(({ id, data }) => (
                <div key={id} className="flex flex-col gap-2 border-b border-zinc-100 p-4 sm:flex-row sm:items-center">
                  <div className="min-w-0 flex-1">
                    <Link
                      href={`/rooms/${id}/${roomSlug({ title: data.title, area: data.area, genderAllowed: data.genderAllowed, rent: data.rent })}`}
            ...
            <div className="card mt-3 overflow-hidden">
              {items.map(({ id, data }) => (
                <div key={id} className="flex flex-col gap-2 border-b border-zinc-100 p-4 sm:flex-row sm:items-center">
                  <div className="min-w-0 flex-1">
                    <Link
                      href={`/items/${id}/${itemSlug({ title: data.title, category: data.category, area: data.area, price: data.price })}`}
                      className="truncate text-sm font-medium hover:underline"
                    >
                      {data.title}
                    </Link>
