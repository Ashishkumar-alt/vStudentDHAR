...
const schema = z.object({
  ...
  walkMinutesToHPU: z
    ...
    .optional()
    .or(z.literal(""))
    .refine((v) => v == null || v === "" || (Number.isFinite(Number(v)) && Number(v) >= 0 && Number(v) <= 120), "0-120"),
  sunFacing: z.boolean(),
          ...
          <>
            {(listing.photoUrls || []).length ? (
              <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {listing.photoUrls.slice(0, 8).map((url) => (
                ...
                <p className="mt-3 text-xs text-zinc-500">
                  To replace photos, pick new ones below (max 5). Old photos will stay in Cloudinary but wonâ€™t show in the listing.
                </p>
              </div>
            ) : null}
            ...
            <form className="card mt-6 p-5" onSubmit={form.handleSubmit(onSubmit)}>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="text-sm font-medium">Rent (â‚¹/month)</label>
                  <input className="input mt-2" inputMode="numeric" {...form.register("rent")} />
                </div>
                <div>
                  <label className="text-sm font-medium">Deposit (â‚¹)</label>
                ...
                <div>
                  ...
                  {form.formState.errors.walkMinutesToHPU ? <p className="mt-1 text-xs text-red-600">{form.formState.errors.walkMinutesToHPU.message}</p> : null}
                </div>
